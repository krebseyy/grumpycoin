...
#if defined(NDEBUG)
# error "Grumpycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfc7989ae0cf5d59b4e953df03b59ba6b406de4c3c05a8f2490b896b3d2698d45");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Grumpycoin: starting difficulty is 1 / 2^12
...
{
    // Grumpycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Grumpycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Grumpycoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Grumpycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Grumpycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Grumpycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xaf62fde8094bf55df4aad0436b36ecedc22342096e7d2912ef304f74d7d16c3b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jody Wilson-Raybould won't run in next election";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042487e5e72b3138ad8b5bc07a3d19503c38d43c78b42ed16a9d64891974936dc10f739f5ed7a6a24e083ca621bdaed2c62da12e21dbd842b0fcb07b0522d7482b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1625855153;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084608049;

        if (fTestNet)
        {
            block.nTime    = 1625855118;
            block.nNonce   = 386748280;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x36e8a02a5a1abd00fee29dbf46480f2bf74e3d1174256769542f2e376acfb2ea"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc4, 0xb8, 0xd5 }; // Grumpycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GrumpycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GrumpycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GrumpycoinMiner(CWallet *pwallet)
{
    printf("GrumpycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("grumpycoin-miner");
    ...
    try { loop {
        ...
        printf("Running GrumpycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GrumpycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GrumpycoinMiner, pwallet));
}
